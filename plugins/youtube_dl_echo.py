...
from database.access import clinton as db
from translation import Translation
from database.adduser import AddUser
from pyrogram import Client as LazyDeveloperr
...
from lazyDeveloperr.ran_text import random_char
from pyrogram.types import Thumbnail


@LazyDeveloperr.on_message(filters.private & filters.regex(pattern=".*http.*"))
async def echo(client, message):
    if Env.LOG_CHANNEL:
        try:
            log_message = await message.forward(Env.LOG_CHANNEL)
    ...
    user_data = await db.is_user_exist(usr_id)
    if not user_data:
    ...
    try:
        ...
        if t_response:
            ...
            save_ytdl_json_path = Env.DOWNLOAD_LOCATION + \
            ...
            await client.send_message(
                chat_id=message.chat.id,
                text= f"⏯<b>File Name:</b> {namee}\n\n🧬<b>File Size:</b> {size} \n<b>⩙ Upload Type:</b> {upload_type}" + Translation.FORMAT_SELECTION.format(Thumbnail) + "\n" + Translation.SET_CUSTOM_USERNAME_PASSWORD,

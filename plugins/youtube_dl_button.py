...
from translation import Translation as script
...
async def youtube_dl_call_back(client, query):
    ...
    tmp_directory_for_each_user = Env.DOWNLOAD_LOCATION + "/" + str(query.from_user.id) + f'{random1}'
    ...
    if tg_send_type == "audio":
        command_to_exec = [
            "yt-dlp",
            "-c",
            "--max-filesize", str(Env.TG_MAX_FILE_SIZE),
            "--prefer-ffmpeg",
            "--extract-audio",
    ...
    if Env.HTTP_PROXY != "":
        ...
        command_to_exec.append(Env.HTTP_PROXY)
    if youtube_dl_username is not None:
    ...
    if t_response:
        ...
        file_size = Env.TG_MAX_FILE_SIZE + 1
        try:
            ...
            file_size = os.stat(download_directory).st_size
        if file_size > Env.TG_MAX_FILE_SIZE:
            await query.edit_message_text(
